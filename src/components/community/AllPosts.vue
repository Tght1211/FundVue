  ...
  <div style="width: 100%">
    ...
    <el-dialog v-model="dialogVisible" width="70%">
      ...
      <div>
        ...
        >
          <div
            v-if="detail.userId == user.userId"
            style="display: flex; color: #50a0ff"
          >
            <div style="float: right; margin-right: 50px" @click="editPost()">
              <span>编辑帖子</span>
            </div>
            <div style="float: right" @click="deletePost()">
              <span>删除帖子</span>
            </div>
          </div>
        </div>
                ...
                <div style="margin-top: 15px; font-size: 15px">
                  ...
                  <span
                    style="
                      ...
                      margin-left: 10px;
                    "
                    v-if="itcom.userId == detail.userId"
                    >&nbsp;作者&nbsp;</span
                  >
                </div>
                <!-- 内容  20px的字-->
                ...
                <form ref="form" :model="form">
                  ...
                  >
                    ...
                    >
                      <!-- 顶下去 -->
                      <div
                        style="height: 50px; background-color: #3c3f41"
                      ></div>
                      <!-- 头部信息 -->
                      <div
                        style="
...
export default {
  ...
  mounted() {
    that.$nextTick(function () {
      ...
      that.editor = new E("#div2");
      // 配置 server 接口地址  // 添加图片
      that.editor.config.uploadImgServer = "http://www.yjzs.gold:10010/com/posts/editor";
      that.editor.config.uploadFileName = "file"; // 设置上传参数名称

      // 设置编辑区域高度为 500px
      that.editor.config.height = 500;
      // 配置颜色（文字颜色、背景色）
      that.editor.config.colors = ["#000000", "#eeece0", "#1c487f", "#4d80bf"];
      // 配置字体
      that.editor.config.fontNames = [
        // 对象形式 v4.6.16
        { name: "黑体", value: "黑体" },
  ...
  methods: {
    ...
    deletePost() {
      console.log(this.detail);
      request.delete("/com/posts/delete/" + this.detail.posId).then((res) => {
        if (res.code == 0) {
          this.$message({
            type: "success",
            message: "删除帖子成功",
    ...
    replay3() {
      if (this.addComMin2.comContent == "") {
        ...
        this.addComMin2.comConId = this.comcon.comId; // 主评论关联 id
        this.addComMin2.posId = this.comconZ.posId;
        this.addComMin2.comType =
          this.comconZ.comId + "@" + this.comconZ.userId; //子评论id+@+子评论用户id。
    ...
    info(index) {
      ...
      this.detail.posType = this.count[index].posType;
      ...
      request
        ...
        .then((res) => {
          if (res.code == 0) {
            ...
            if (this.countLeng >= this.total && this.countLeng != 0) {
              this.$message({
                type: "warning",
                message: "加载完啦~",
              });
              ...
              sessionStorage.setItem("PostsTotal", this.total);
